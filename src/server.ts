...
const proc = (q: express.Request, r: express.Response, entity: string, operation: string) => {
  (expr_svc as any)[entity][operation]
    .invoke({ ...q.body, ...q.query, ...q.params })
    ...
    .catch((err: any) => {
      ...
      if (err.body) {
        try {
          message = JSON.parse(err.body).detail;
        } catch (ex) {
          console.warn('Failed to get body detail', err.body, ex);
        }
      }
...
db.syncStructure().then(() => {
  ...
  expr_api.get('/', (q, r) =>
    r.render(path.resolve(__dirname, '../ui/index.html'), {
      acmeTerms: apiConfig.acmeTerms,
      recaptcha: process.env['acme::recaptcha::public'],
    })
  );
  expr_api.get('/main.js', (q, r) =>
    r.render(path.resolve(__dirname, '../ui/main.js'), {
  ...
  expr_api.get('/order/:orderId/cert/:certCode/:certType/:password?/:friendlyName?', (q, r) =>
    sec_proc(q, r, 'orders', 'cert')
  );
