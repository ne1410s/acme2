((window) => {
  ...
  async function svc(secure, path, method, json) {
    ...
    if (response.status === 401 && secure === true) {
      ...
      return obtain_login(true).then(() => svc(secure, path, method, json));
    } else if (!response.ok) {
      throw resultjson;
    }
  ...
  function obtain_login(fresh) {
    return new Promise((resolve) => {
      ...
      q2f('[type=button]', modal).onclick = () => {
        ...
        grecaptcha.execute(recaptchaKey, { action: 'login' }).then((recaptcha) =>
          svc(false, 'login', 'POST', { username, password, recaptcha })
            .then((authJson) => {
              save_token(authJson);
            ...
            .catch((err) => {
              errors.innerHTML = err.detail ? err.detail.join('<br>') : err.message || err;
            })
  ...
  function obtain_registration() {
    return new Promise((resolve) => {
      ...
      q2f('[type=button]', modal).onclick = () => {
        ...
        // obtain recaptcha token for server-side verification
        grecaptcha.execute(recaptchaKey, { action: 'register' }).then((recaptcha) =>
          svc(false, 'user', 'POST', { username, password, recaptcha })
            .then((authJson) => {
              save_token(authJson);
              modal.classList.remove('open');
              q2f('body').classList.add('auth');
    ...
    return new Promise((resolve) => {
      ...
      if (isUpdating) {
        ...
        btnDelete.onclick = (event) => {
          ...
          if (
            confirm(
              'You are about to permanently delete the account and all associated orders. This action cannot be undone. Continue?'
            )
          ) {
            svc(true, `account/${existing.accountId}`, 'DELETE')
              ...
              .catch((err) => {
                txtErrors.innerHTML = err.detail ? err.detail.join('<br>') : err.message || err;
              });
          }
        };
      }
      ...
      q2f('[type=button]', modal).onclick = () => {
        ...
        const svc_verb = isUpdating ? 'PUT' : 'POST',
          emails = txtEmails.value.split(/[\s,;]/g).filter((a) => a.length !== 0);

        svc(true, 'account', svc_verb, {
          accountId,
          emails,
          tosAgreed: chkTosAgreed.checked,
  ...
  function obtain_add_order(accountId) {
    return new Promise((resolve) => {
      ...
      q2f('[type=button]', modal).onclick = () => {
        ...
        const domains = q2f('[placeholder=domains]', modal)
          .value.split(/[\s,;]/g)
          .filter((a) => a.length !== 0);
          ...
          switch (order.status) {
            ...
            case 'valid':
              const downloader = q2f('.downloader', modal),
                ...
                certTypeChange = (event) =>
                  modal.setAttribute('data-cert-type', event.target.value);
                ...
                svc(true, getCertUrl, 'GET')
                  .then((json) => {
                    ...
                    downloader.setAttribute(
                      'href',
                      `data:${json.contentType};charset=utf-8;base64,${json.base64}`
                    );
                    downloader.click();
                  })
                  .finally(() => modal.classList.remove('loading'));
                  ...
                  switch (challengeType) {
                    case 'dns-01':
                      ...
                      challengeDesc.innerHTML =
                        'This challenges requires you to add a record in DNS';
                      dnsMat1.innerHTML = 'Record type: TXT';
                      dnsMat2.innerHTML =
                        'Name: ' + '<b>_acme-challenge</b><i>.' + domainClaim.domain + '</i>';
                    ...
                    case 'http-01':
                      ...
                      textLink.setAttribute(
                        'href',
                        `data:application/octet-stream;charset=utf-8,${challenge.content}`
                      );
                      ...
                      testLink.onclick = (event) => {
                        ...
                        fetch(absoluteUrl)
                          .then((res) => {
                            res
                              .text()
                              .then((text) => alert(text === challenge.content ? 'pass' : 'fail'));
                          })
                          .catch((err) => {
                            console.warn(err);
                            alert('fail');
                      ...
                      challengeDesc.innerHTML =
                        'This challenge requires you to serve text over HTTP';
  ...
  window.addEventListener('load', () => {
    ...
    q2a('.secure > .modal > .body').forEach(
      (m) => (m.onclick = (event) => event.stopPropagation())
    );
